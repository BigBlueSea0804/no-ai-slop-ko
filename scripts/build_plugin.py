#!/usr/bin/env python3
"""Build and validate the distributable No AI Slop Korean plugin archive."""
...
def validate_source(manifest: dict) -> None:
    ...
    required_sources = (
        ...
        SKILL_ROOT / "references" / "korean-patterns.md",
...
def build_plugin(manifest: dict) -> tuple[Path, Path]:
    ...
    archive = DIST / f"{SKILL_NAME}-plugin-{manifest['version']}.zip"
    if archive.exists():
        archive.unlink()
...
def validate_build(plugin_root: Path, archive: Path) -> None:
    expected = {
        ...
        "skills/no-ai-slop-ko/SKILL.md",
        "skills/no-ai-slop-ko/eval.md",
        "skills/no-ai-slop-ko/agents/openai.yaml",
        "skills/no-ai-slop-ko/references/korean-patterns.md",
        "LICENSE",
        "PRIVACY.md",
        "TERMS.md",
    }
    ...
    packaged_eval = plugin_root / "skills" / SKILL_NAME / "eval.md"
    packaged_patterns = plugin_root / "skills" / SKILL_NAME / "references" / "korean-patterns.md"
    ...
    if packaged_patterns.read_bytes() != (SKILL_ROOT / "references" / "korean-patterns.md").read_bytes():
        raise SystemExit("Packaged Korean patterns do not match the canonical file")
